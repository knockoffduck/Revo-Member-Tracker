...
export default function GymPreferences() {
    ...
    const { data: allGymsData, error: allGymsError, isLoading: isLoadingAllGyms } = useSWR("/api/db/gyminfo", fetchGyms);

    // Fetch user's saved preferences using SWR
    // *** Adjust "/api/user/gym-preferences" to your actual endpoint ***
    const { data: userPreferences, error: userPrefsError, isLoading: isLoadingUserPrefs } = useSWR("/api/account/gym-preferences", fetchUserPreferences);
    ...
    const WAGyms: GymInfo[] = allGymsData.filter((gym: GymInfo) => gym.state === "WA");
    const SAGyms: GymInfo[] = allGymsData.filter((gym: GymInfo) => gym.state === "SA");
    const VICGyms: GymInfo[] = allGymsData.filter((gym: GymInfo) => gym.state === "VIC");
    const NSWGyms: GymInfo[] = allGymsData.filter((gym: GymInfo) => gym.state === "NSW");
    const QLDGyms: GymInfo[] = allGymsData.filter((gym: GymInfo) => gym.state === "QLD");
        ...
        <div className="flex w-full px-8 justify-center pt-6 ">
            ...
            <Card className="w-full">
                <CardHeader>
                    ...
                    <CardDescription>Select the gyms you would like to be displayed</CardDescription>
                </CardHeader>
                <CardContent>{ }
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            <FormField
                                ...
                                render={() => (
                                    <FormItem>
                                        <FormLabel className="text-lg font-semibold">Western Australia</FormLabel>
                                        {WAGyms.map((gym) => (
                                            <FormField
                                                key={gym.name}
                                                control={form.control}
                                                            ...
                                                            <FormControl>
                                                                <Checkbox
                                                                    ...
                                                                    onCheckedChange={(checked) => {
                                                                        return checked
                                                                            ? field.onChange([...field.value, gym.name])
                                                                            : field.onChange(
                                                                                field.value?.filter(
                            ...
                            <FormField
                                ...
                                render={() => (
                                    <FormItem>
                                        <FormLabel className="text-lg font-semibold">Victoria</FormLabel>
                                        {VICGyms.map((gym) => (
                                            <FormField
                                                key={gym.name}
                                                control={form.control}
                            ...
                            <FormField
                                ...
                                render={() => (
                                    <FormItem>
                                        <FormLabel className="text-lg font-semibold">New South Wales</FormLabel>
                                        {NSWGyms.map((gym) => (
                                            <FormField
                                                key={gym.name}
                                                control={form.control}
                            ...
                            <FormField
                                ...
                                render={() => (
                                    <FormItem>
                                        <FormLabel className="text-lg font-semibold">South Australia</FormLabel>
                                        {SAGyms.map((gym) => (
                                            <FormField
                                                key={gym.name}
                                                control={form.control}
