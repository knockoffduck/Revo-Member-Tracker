...
Repo: Revo Member Tracker (Next.js 15 + Bun + PocketBase).
...
- `app/db/database.ts` — PocketBase client (server-side).
- `lib/auth.ts` — lightweight PocketBase client export.
- `lib/auth-client.ts` — client-side auth hook + sign-out helper using PocketBase SDK.
- `lib/authz.ts` — `getSessionOrThrow()`, `requireAdminSession()` for server actions/route handlers.
- `lib/pocketbase.ts` — shared types and browser PocketBase client.
- `lib/server/pocketbase.ts` — server-side PocketBase helpers: admin auth, session decoding, cookie management.
...
- **PocketBase is the single source of truth.** The old `app/db/schema.ts`, `drizzle/`, and `auth-schema.ts` files have been removed. All reads/writes go through the PocketBase SDK (`lib/server/pocketbase.ts` on the server, `lib/pocketbase.ts` in the browser).
- **Auth is now PocketBase auth.** The Better Auth dependency and `/api/auth/[...all]` route have been removed. Users are stored in PocketBase's built-in `users` collection with custom fields `is_admin` and `gym_preferences`. The `pb_auth` cookie is used for session state.
...
- **Docker**: multi-stage `Dockerfile` (oven/bun:1-slim → node:22-slim) uses `output: "standalone"`. Build args needed: `NEXT_PUBLIC_BASE_URL`, `NEXT_PUBLIC_ADMIN_API_URL`, `NEXT_PUBLIC_POCKETBASE_URL`, `POCKETBASE_URL`, `POCKETBASE_ADMIN_EMAIL`, `POCKETBASE_ADMIN_PASSWORD`. `NEXT_TELEMETRY_DISABLED=1` is set in build + runtime. Exposes `:3000`, starts with `node server.js`.
